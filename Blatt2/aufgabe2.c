...
void V(int sem_id,int l, int r) {
  struct sembuf semaphore[2];

  semaphore[0].sem_num = l;
  semaphore[1].sem_num = r;
...
#define NUM_CHILD_LOOPS 2

for(int i = 0; i < NUM_CHILD_LOOPS; ++i)
{
    int denkenTime = rand() % 2 + 1;
    int essenTime = rand() % 2 + 1;
